...
fn do_ibc_packet_receive(
    ...
    // If memo is set we need to forward the information to our contract to action
    // else we just send the tokens to the receiver
    let mut submsg = if let Some(received_memo) = &msg.memo {
        let config = CONFIG.load(deps.storage)?;

        // If no hook contract is set but we received a memo, fail and return funds
        // we do this as a safety feature - otherwise a user could send a memo to be actioned
        // on the Hub, but the contract isn't set which would result in a loss of funds.
        // In case a user just wants to add a memo, the contract would pass the funds on
        // to the final destination
        let hook_contract = config.hook_addr.ok_or(ContractError::NoHookContract {})?;

        let action_msg: CosmosMsg = match to_send {
            // If someone sends native tokens to our channel, fail the transfer
            // as native tokens should use the transfer<>transfer channels
            Amount::Native(..) => Err(ContractError::NotOnAllowList {}),
            // All CW20's on our allowed list is forwarded to the handler contract
            // it is up to the handler to decide if the CW20 is allowed or not
            Amount::Cw20(coin) => {
                let msg = Cw20ExecuteMsg::Send {
                    contract: hook_contract.to_string(),
                    amount: coin.amount,
                    msg: to_json_binary(&Cw20HookMsg::OutpostMemo {
                        // The channel this packet was received on
                        channel: packet.dest.channel_id.clone(),
                        // Original sender from Outpost
                        sender: msg.sender.clone(),
                        // Original receiver set in the transfer
                        receiver: msg.receiver.clone(),
                        memo: received_memo.clone(),
                    })?,
                };
        ...
        SubMsg::reply_on_error(action_msg, RECEIVE_ID)
    } else {
        // Memo is not set, send to receiver
        let send = send_amount(to_send, msg.receiver.clone());
        SubMsg::reply_on_error(send, RECEIVE_ID)
    };

    submsg.gas_limit = gas_limit;

    // make sure we have enough balance for this
    // We can't update the channel balance before checking the memo as in
    // the original contract since ContractErrors are handled as Ok to pass
    // back the ack. See original ibc_packet_receive for more info
...
fn on_packet_failure(
    ...
    let ics_msg: Ics20Packet = from_json(&packet.data)?;
    ...
    let to_send = Amount::from_parts(ics_msg.denom.clone(), ics_msg.amount);
    let gas_limit = check_gas_limit(deps.as_ref(), &to_send)?;

    // If the sender of this was originally the hook contract, we need
    // to send the funds back via notification instead of just sending
    // the funds
    let config = CONFIG.load(deps.storage)?;
    let msg: CosmosMsg;
    if let Some(hook_addr) = &config.hook_addr {
        // Hook contract was the sender
        if hook_addr == &ics_msg.sender {
            msg = match to_send {
                // Native tokens via the channel is not allowed
                Amount::Native(_coin) => Err(ContractError::NotOnAllowList {}),
                // Notify the memo handler of the failure
                Amount::Cw20(coin) => {
                    let cw20_msg = Cw20ExecuteMsg::Send {
                        contract: hook_addr.to_string(),
                        amount: coin.amount,
                        msg: to_json_binary(&Cw20HookMsg::TransferFailure {
                            receiver: ics_msg.receiver.clone(),
                        })?,
                    };
            ...
            // Hook contract was not the sender, send funds back to sender
            msg = send_amount(to_send, ics_msg.sender.clone());
        }
    } else {
        // No hook is set, send funds back to sender
        msg = send_amount(to_send, ics_msg.sender.clone());
    }
    let mut submsg = SubMsg::reply_on_error(msg, ACK_FAILURE_ID);
    submsg.gas_limit = gas_limit;
    ...
    let res = IbcBasicResponse::new()
        ...
        .add_attribute("sender", ics_msg.sender)
        .add_attribute("receiver", ics_msg.receiver)
        .add_attribute("denom", ics_msg.denom)
        .add_attribute("amount", ics_msg.amount.to_string())
...
mod test {
    ...
    fn send_receive_cw20() {
        ...
        let recv_packet =
            mock_receive_packet(send_channel, 876543210, cw20_denom, "local-rcpt", None);
    ...
    #[test]
    fn send_receive_cw20_no_handler_with_memo() {
        let send_channel = "channel-9";
        let cw20_addr = "token-addr";
        let cw20_denom = "cw20:token-addr";
        let gas_limit = 1234567;
        ...
        let recv_packet =
            mock_receive_packet(send_channel, 876543210, cw20_denom, "local-rcpt", None);
        let recv_packet_with_memo = mock_receive_packet(
            send_channel,
            876543210,
            cw20_denom,
            "local-rcpt",
            Some("Sample memo".to_string()),
        );
        let recv_high_packet =
            mock_receive_packet(send_channel, 1876543210, cw20_denom, "local-rcpt", None);
        ...
        let ack: Ics20Ack = from_json(&res.acknowledgement).unwrap();
        // We should get an error because no handler is set
        assert!(matches!(ack, Ics20Ack::Error(_)));

        // query channel state
        // The balance in the channels should not have changed due to failure
        let state = query_channel(deps.as_ref(), send_channel.to_string()).unwrap();
        assert_eq!(state.balances, vec![Amount::cw20(987654321, cw20_addr)]);
        assert_eq!(state.total_sent, vec![Amount::cw20(987654321, cw20_addr)]);
    }
    ...
    fn send_receive_native() {
        ...
        // prepare some mock packets
        let recv_packet = mock_receive_packet(send_channel, 876543210, denom, "local-rcpt", None);
        let recv_high_packet =
    ...
    fn check_gas_limit_handles_all_cases() {
        ...
        let mut deps = setup_standard_0134(&[send_channel], &[(allowed, allowed_gas)]);
