...
// Instantiate using the original CW20-ICS20 contract version 0.13.4
pub fn setup_standard_0134(
    channels: &[&str],
    allow: &[(&str, u64)],
) -> OwnedDeps<MockStorage, MockApi, MockQuerier> {
    let mut deps = mock_dependencies();
    ...
    let allowlist = allow
        .iter()
        .map(|(contract, gas)| cw20_ics20_original::msg::AllowMsg {
            contract: contract.to_string(),
            gas_limit: Some(*gas),
        })
        .collect();
    ...
    // instantiate an empty contract
    let instantiate_msg = cw20_ics20_original::msg::InitMsg {
        default_gas_limit: None,
        default_timeout: DEFAULT_TIMEOUT,
        gov_contract: "gov".to_string(),
        allowlist,
    ...
    let info = mock_info(&String::from("anyone"), &[]);
    let res = cw20_ics20_original::contract::instantiate(
        deps.as_mut(),
        mock_env(),
        info,
        instantiate_msg,
