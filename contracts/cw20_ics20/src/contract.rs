...
pub fn instantiate(
    ...
    let cfg = Config {
        ...
        hook_addr: addr_opt_validate(deps.api, &msg.hook_addr)?,
    };
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateHookAddress { new_address } => {
            execute_update_hook_address(deps, info, new_address)
        }
    }
}
...
/// Update the hook contract address to the new one provided
/// May only be executed by the contract admin
pub fn execute_update_hook_address(
    deps: DepsMut,
    info: MessageInfo,
    new_address: String,
) -> Result<Response, ContractError> {
    ...
    let validated_address = deps.api.addr_validate(&new_address)?;
    let mut cfg = CONFIG.load(deps.storage)?;
    cfg.hook_addr = Some(validated_address);
    CONFIG.save(deps.storage, &cfg)?;

    Ok(Response::default()
        .add_attribute("action", "update_hook_contract")
...
pub fn migrate(deps: DepsMut, _env: Env, msg: MigrateMsg) -> Result<Response, ContractError> {
    ...
    if storage_version <= MIGRATE_VERSION_ASTROPORT_V1.parse().map_err(from_semver)? {
        ...
        let config = Config {
            ...
            default_gas_limit: old_config.default_gas_limit,
            hook_addr: addr_opt_validate(deps.api, &msg.hook_addr)?,
        };
...
mod test {
    ...
    fn execute_cw20_fails_if_not_whitelisted_unless_default_gas_limit() {
        let send_channel = "channel-15";
        let mut deps = setup_standard_0134(&[send_channel], &[]);
    ...
    fn astroport_v1_migration_works() {
        ...
        let send_channel = "channel-15";
        let cw20_addr = "my-token";
        // Instantiate the original v0.13.4 contract
        let mut deps = setup_standard_0134(&[send_channel], &[(cw20_addr, 123456)]);
        ...
        migrate(
            ...
            MigrateMsg {
                default_gas_limit: Some(123456),
                hook_addr: Some("hook_contract".to_string()),
            },
        )
        .unwrap();
        ...
        assert_eq!(config.hook_addr.unwrap(), "hook_contract");
    }
    ...
    #[test]
    fn update_hook_contract() {
        let mut deps = setup(&["channel-0"], &[]);

        let msg = ExecuteMsg::UpdateHookAddress {
            new_address: "new_hook_contract".to_string(),
        };
        // Attempt to update while not Admin
        let info = mock_info("not_admin", &coins(1234567, "ucosm"));
        let res = execute(deps.as_mut(), mock_env(), info, msg.clone()).unwrap_err();

        assert_eq!(res, ContractError::Admin(AdminError::NotAdmin {}));

        // Update while Admin
        let info = mock_info("gov", &coins(1234567, "ucosm"));
        execute(deps.as_mut(), mock_env(), info, msg).unwrap();

        // Ensure new handler is set
        let config = query_config(deps.as_ref()).unwrap();
        assert_eq!(config.hook_addr.unwrap(), "new_hook_contract");
    }
}
