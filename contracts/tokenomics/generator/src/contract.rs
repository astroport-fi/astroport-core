...
pub fn instantiate(
    ...
    for proxy in msg.allowed_reward_proxies {
        allowed_reward_proxies.push(user_input_to_addr(deps.api, &proxy)?);
    }

    let config = Config {
        owner: user_input_to_addr(deps.api, &msg.owner)?,
        astro_token: user_input_to_addr(deps.api, &msg.astro_token)?,
        tokens_per_block: msg.tokens_per_block,
        total_alloc_point: Uint64::from(0u64),
        start_block: msg.start_block,
        allowed_reward_proxies,
        vesting_contract: user_input_to_addr(deps.api, &msg.vesting_contract)?,
    };
...
pub fn execute_update_config(
    ...
    if let Some(vesting_contract) = vesting_contract {
        config.vesting_contract = user_input_to_addr(deps.api, vesting_contract.as_str())?;
    }
...
pub fn add(
    ...
    let lp_token = user_input_to_addr(deps.api, lp_token.as_str())?;
    ...
    let reward_proxy = reward_proxy
        .map(|v| user_input_to_addr(deps.api, &v))
        .transpose()?;
...
pub fn deposit(
    ...
    let lp_token = user_input_to_addr(deps.api, lp_token.as_str())?;
    let beneficiary = user_input_to_addr(deps.api, beneficiary.as_str())?;
...
fn send_orphan_proxy_rewards(
    ...
    let lp_token = user_input_to_addr(deps.api, &lp_token)?;

    let recipient = user_input_to_addr(deps.api, &recipient)?;
...
pub fn query_deposit(deps: Deps, lp_token: Addr, user: Addr) -> Result<Uint128, ContractError> {
    let lp_token = user_input_to_addr(deps.api, lp_token.as_str())?;
    let user = user_input_to_addr(deps.api, user.as_str())?;
