...
use crate::state::{
    capture_total_supply_history, check_sender_is_minter, get_total_supply_at, BALANCES,
};
use astroport::asset::addr_opt_validate;
use astroport::xastro_outpost_token::{MigrateMsg, QueryMsg};
...
const CONTRACT_NAME: &str = "astroport-xastro-outpost-token";
...
pub fn execute_burn(
    ...
    check_sender_is_minter(&info.sender, &config)?;
...
///
/// * **BalanceAt { address, timestamp }** Returns the balance of the given address at the given timestamp
/// using a [`BalanceResponse`] object.
///
/// * **TotalSupplyAt { timestamp }** Returns the total supply at the given timestamp.
///
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::BalanceAt { address, timestamp } => {
            to_json_binary(&query_balance_at(deps, address, timestamp)?)
        }
        QueryMsg::TotalSupplyAt { timestamp } => {
            to_json_binary(&get_total_supply_at(deps.storage, timestamp)?)
        }
...
pub fn migrate(_deps: DepsMut, _env: Env, _msg: MigrateMsg) -> StdResult<Response> {
    Err(StdError::generic_err(
        "Cannot migrate. No migrations available",
    ))
}
