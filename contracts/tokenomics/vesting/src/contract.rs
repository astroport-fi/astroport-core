...
pub fn instantiate(
    ...
    CONFIG.save(
        ...
        &Config {
            ...
            token_addr: user_input_to_addr(deps.api, &msg.token_addr)?,
        },
    )?;
...
pub fn update_config(
    ...
    if let Some(owner) = owner {
        config.owner = user_input_to_addr(deps.api, &owner)?;
    }
...
pub fn register_vesting_accounts(
    ...
    for mut vesting_account in vesting_accounts {
        ...
        let account_address = user_input_to_addr(deps.api, &vesting_account.address)?;
...
pub fn query_vesting_account(deps: Deps, address: Addr) -> StdResult<VestingAccountResponse> {
    let address = user_input_to_addr(deps.api, address.as_str())?;
