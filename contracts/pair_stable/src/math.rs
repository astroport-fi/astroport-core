...
///
pub(crate) fn compute_d(leverage: Uint64, pools: &[Decimal256]) -> StdResult<Decimal256> {
    let amount_a_times_coins = (pools[0] * N_COINS).checked_add(Decimal256::one())?;
    let amount_b_times_coins = (pools[1] * N_COINS).checked_add(Decimal256::one())?;
    ...
    if sum_x.is_zero() {
        ...
        for _ in 0..ITERATIONS {
            let mut d_product = d;
            d_product = d_product.checked_multiply_ratio(d, amount_a_times_coins)?;
            d_product = d_product.checked_multiply_ratio(d, amount_b_times_coins)?;
            d_previous = d;
            // d = (leverage * sum_x + d_p * n_coins) * d / ((leverage - 1) * d + (n_coins + 1) * d_p);
            d = calculate_step(d, leverage.u64(), sum_x, d_product)?;
...
fn calculate_step(
    ...
    let leverage_mul = Decimal256::from_ratio(leverage, AMP_PRECISION).checked_mul(sum_x)?;
    let d_p_mul = d_product.checked_mul(N_COINS)?;

    let l_val = leverage_mul.checked_add(d_p_mul)?.checked_mul(initial_d)?;

    let leverage_sub = initial_d.checked_mul(Decimal256::from_ratio(
        leverage - AMP_PRECISION,
        AMP_PRECISION,
    ))?;
