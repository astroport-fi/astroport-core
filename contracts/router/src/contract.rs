...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            astroport_factory: user_input_to_addr(deps.api, &msg.astroport_factory)?,
        },
    )?;
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::AssertMinimumReceive {
            ...
            prev_balance,
            minimum_receive,
            user_input_to_addr(deps.api, &receiver)?,
        ),
    }
}
...
pub fn receive_cw20(
    ...
    let sender = user_input_to_addr(deps.api, &cw20_msg.sender)?;
    match from_binary(&cw20_msg.msg)? {
        Cw20HookMsg::ExecuteSwapOperations {
            ...
            let to_addr = if let Some(to_addr) = to {
                Some(user_input_to_addr(deps.api, to_addr.as_str())?)
            } else {
                None
            };
...
pub fn execute_swap_operations(
    ...
    let to = if let Some(to) = to {
        user_input_to_addr(deps.api, to.as_str())?
    } else {
        sender
    };
