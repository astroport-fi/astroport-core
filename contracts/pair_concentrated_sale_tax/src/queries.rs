...
use astroport_pcl_common::utils::{
    accumulate_prices, before_swap_check, calc_last_prices, get_share_in_assets,
};
use astroport_pcl_common::{calc_d, get_xcp};

use crate::contract::LP_TOKEN_PRECISION;
use crate::error::ContractError;
use crate::state::{BALANCES, CONFIG, OBSERVATIONS, TAX_CONFIG};
use crate::utils::{
    calculate_shares, compute_offer_amount_sale_tax, compute_swap_sale_tax,
    get_assets_with_precision, pool_info, query_pools, SwapResultSaleTax,
};
...
pub fn query_simulation(
    ...
    let SwapResultSaleTax { swap_result, .. } = compute_swap_sale_tax(
        &xs,
...
pub fn query_reverse_simulation(
    ...
    let offer_asset = &pools[offer_ind].info;
    let offer_asset_prec = precisions.get_precision(offer_asset)?;

    let xs = pools.iter().map(|asset| asset.amount).collect_vec();

    let stored_tax_config = TAX_CONFIG.load(deps.storage)?;
    let tax_config = stored_tax_config.tax_configs.get(&offer_asset.to_string());

    let (offer_amount, spread_amount, commission_amount) = compute_offer_amount_sale_tax(
        &xs,
        ask_asset_dec.amount,
        ask_ind,
        &config,
        &env,
        tax_config,
...
pub fn query_config(deps: Deps, env: Env) -> StdResult<ConfigResponse> {
    let config = CONFIG.load(deps.storage)?;
    let tax_configs = TAX_CONFIG.load(deps.storage)?;
    let amp_gamma = config.pool_state.get_amp_gamma(&env);
    ...
    Ok(ConfigResponse {
        ...
        params: Some(to_json_binary(&ConcentratedPoolConfigSaleTax {
            amp: amp_gamma.amp,
            ...
            tax_config_admin: tax_configs.tax_config_admin.to_string(),
            tax_configs: tax_configs.tax_configs.into(),
        })?),
