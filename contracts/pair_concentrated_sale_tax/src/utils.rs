...
#[derive(Debug)]
pub struct SwapResultSaleTax {
    pub swap_result: SwapResult,

    pub offer_amount_dec: Decimal256,
...
) -> StdResult<SwapResultSaleTax> {
    // Deduct tax
    let mut offer_amount_dec = offer_amount;
    let sale_tax_dec = if let Some(tax_config) = tax_config {
        let tax_rate: Decimal256 = tax_config.tax_rate.into();
        let sale_tax = tax_rate * offer_amount_dec;
        offer_amount_dec = offer_amount_dec.checked_sub(sale_tax)?;
        sale_tax
    } else {
        Decimal256::zero()
    };

    Ok(SwapResultSaleTax {
        swap_result: compute_swap(
            xs,
            offer_amount_dec,
            ask_ind,
            config,
...
pub fn compute_offer_amount_sale_tax(
    xs: &[Decimal256],
    want_amount: Decimal256,
    ask_ind: usize,
    config: &Config,
    env: &Env,
    tax_config: Option<&TaxConfigChecked>,
    ...
    let (mut offer_amount, spread_amount, commission_amount) =
        compute_offer_amount(xs, want_amount, ask_ind, config, env)?;
    ...
    if let Some(tax_config) = tax_config {
        ...
        offer_amount =
            offer_amount.checked_mul(Decimal256::one() / (Decimal256::one() - tax_rate))?;
    }
