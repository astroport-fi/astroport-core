...
use astroport::pair_concentrated_sale_tax::{
    ConcentratedPoolParamsSaleTax, ConcentratedPoolUpdateParamsSaleTax, StoredTaxConfig,
};
...
use crate::utils::{
    accumulate_swap_sizes, calculate_shares, compute_swap_sale_tax, get_assets_with_precision,
    query_pools, SwapResultSaleTax,
};
...
fn swap(
    ...
    let stored_tax_config = TAX_CONFIG.load(deps.storage)?;
    let tax_config = stored_tax_config
        .tax_configs
        .get(&offer_asset.info.to_string());

    let SwapResultSaleTax {
        swap_result,
        offer_amount_dec,
        sale_tax_dec,
    } = compute_swap_sale_tax(
        &xs,
    ...
    let sale_tax = sale_tax_dec.to_uint(offer_asset_prec)?;
    let offer_amount = offer_amount_dec.to_uint(offer_asset_prec)?;

    xs[offer_ind] += offer_amount_dec;
    ...
    if (swap_result.dy + swap_result.maker_fee + swap_result.share_fee) >= MIN_TRADE_SIZE
        && offer_amount_dec >= MIN_TRADE_SIZE
    {
        let last_price = swap_result.calc_last_price(offer_amount_dec, offer_ind);
    ...
    if let Some(tax_config) = tax_config {
        if !sale_tax.is_zero() {
            messages.push(
                ...
                .into_msg(tax_config.tax_recipient.to_string())?,
            );
        }
    }
    ...
    if offer_amount_dec >= MIN_TRADE_SIZE && swap_result.dy >= MIN_TRADE_SIZE {
    ...
    if config.track_asset_balances {
        BALANCES.save(
            ...
            &(pools[offer_ind].amount + offer_amount_dec).to_uint(offer_asset_prec)?,
            env.block.height,
        )?;
        BALANCES.save(
            deps.storage,
...
fn update_config(
    ...
    let update_params = from_json::<ConcentratedPoolUpdateParamsSaleTax>(&params)?;
    let mut response = Response::default();
    let owner = config.owner.as_ref().unwrap_or(&factory_config.owner);

    match &update_params {
        ConcentratedPoolUpdateParamsSaleTax::UpdateSaleTax(config_updates) => {
            // different auth logic for sale tax updates
            let mut tax_config = TAX_CONFIG.load(deps.storage)?;
            if info.sender != *owner && info.sender != tax_config.tax_config_admin {
            ...
            response.attributes.push(attr("action", "update_sale_tax"));
        }
        _ => {
            ...
            match update_params {
                ...
                ConcentratedPoolUpdateParamsSaleTax::DisableFeeShare => {
