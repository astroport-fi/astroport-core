...
use astroport::pair_xyk_sale_tax::{SaleTaxConfigUpdates, TaxConfig, TaxConfigs};
use cosmwasm_std::{from_json, Addr, Coin, Decimal, Decimal256, StdError, Uint128};
use helper::common_pcl_params;
...
use astroport_pair_concentrated_sale_tax::error::ContractError;
...
use crate::helper::{common_pcl_params_uusdc_tax, AppExtension, Helper};
...
fn check_observe_queries() {
    ...
    let mut helper =
        Helper::new(&owner, test_coins.clone(), common_pcl_params_uusdc_tax()).unwrap();
    ...
    // 99_741246 (without tax)
    assert_eq!(94_754241, helper.coin_balance(&test_coins[0], &user2));

    let d = helper.query_d().unwrap();

    // 200000.260415 (without tax)
    assert_eq!(dec_to_f64(d), 200000.25391591928);
...
fn check_wrong_initialization() {
    ...
    Helper::new(
        &owner,
        vec![TestCoin::native("uluna"), TestCoin::native("uusdc")],
        params,
    )
    .unwrap();
}
...
fn swap_different_precisions() {
    ...
    let test_coins = vec![TestCoin::cw20precise("FOO", 5), TestCoin::native("uusdc")];
...
fn check_reverse_swap() {
    ...
    let test_coins = vec![TestCoin::native("uusdc"), TestCoin::cw20("BAR")];
    ...
    let reverse_sim_resp = helper
        ...
        .unwrap();
    // 50000220879u128 (without tax)
    // as it is hard to predict dynamic fees reverse swap is not exact
    assert_eq!(reverse_sim_resp.offer_amount.u128(), 50000234325u128);
    // 151_913981 (without tax) both smaller expected, as we have less swap
    assert_eq!(reverse_sim_resp.commission_amount.u128(), 146_804859);
    // 16241_558397 (without tax) both smaller expected, as we have less swap
    assert_eq!(reverse_sim_resp.spread_amount.u128(), 14876_920391);
}
...
fn check_swaps_simple() {
    ...
    let test_coins = vec![TestCoin::native("uusdc"), TestCoin::cw20("USDC")];
    ...
    helper.swap(&user, &offer_asset, None).unwrap();
    assert_eq!(0, helper.coin_balance(&test_coins[0], &user));
    // 99_737929 (without tax)
    assert_eq!(94_751169, helper.coin_balance(&test_coins[1], &user));
    ...
    assert_eq!(0, helper.coin_balance(&test_coins[1], &user2));
    // 99_741246 (without tax)
    assert_eq!(99_741118, helper.coin_balance(&test_coins[0], &user2));

    let d = helper.query_d().unwrap();
    // 200000.260415 (without tax)
    assert_eq!(dec_to_f64(d), 200000.2538569159);
...
fn check_amp_gamma_change() {
    ...
    let action = ConcentratedPoolUpdateParamsSaleTax::Update(UpdatePoolParams {
    ...
    let action = ConcentratedPoolUpdateParamsSaleTax::Promote(PromoteParams {
    ...
    let action = ConcentratedPoolUpdateParamsSaleTax::StopChangingAmpGamma {};
    helper.update_config(&owner, &action).unwrap();
    let amp_gamma = helper.query_amp_gamma().unwrap();
...
fn check_prices() {
    ...
    let test_coins = vec![TestCoin::native("uusdc"), TestCoin::cw20("USDX")];
...
fn update_owner() {
    ...
    let mut helper = Helper::new(&owner, test_coins, common_pcl_params_uusdc_tax()).unwrap();
...
fn query_d_test() {
    ...
    let helper = Helper::new(&owner, test_coins.clone(), common_pcl_params_uusdc_tax()).unwrap();
...
fn asset_balances_tracking_with_in_params() {
    ...
    let res = helper
        .query_asset_balance_at(&assets[0].info, helper.app.block_info().height)
        .unwrap();
    // 998_001335 (without tax) - ~5% less taken with tax
    assert_eq!(res.unwrap(), Uint128::new(998_051267));

    let res = helper
        .query_asset_balance_at(&assets[1].info, helper.app.block_info().height)
        .unwrap();
    // 1001_000000 (without tax) - ~5% less received with tax
    assert_eq!(res.unwrap(), Uint128::new(1000_950000));
    ...
    let res = helper
        .query_asset_balance_at(&assets[0].info, helper.app.block_info().height)
        .unwrap();
    // 498_751043 (without tax) - ~5% less taken with tax
    assert_eq!(res.unwrap(), Uint128::new(498_775996));

    let res = helper
        .query_asset_balance_at(&assets[1].info, helper.app.block_info().height)
        .unwrap();
    // 500_249625 (without tax) - ~5% less received with tax
    assert_eq!(res.unwrap(), Uint128::new(500_224638));
}
...
fn provides_and_swaps_and_withdraw() {
    ...
    assert_eq!(res.total_share.u128(), 10);
}
...
fn provide_withdraw_provide() {
    ...
    let test_coins = vec![TestCoin::native("uusdc"), TestCoin::native("uluna")];
...
fn check_correct_fee_share() {
    ...
    let action = ConcentratedPoolUpdateParamsSaleTax::EnableFeeShare {
    ...
    let last_price = helper
        ...
        .price_state
        .last_price;
    // 0.998842355796925899 (without tax)
    assert_eq!(
        last_price,
        Decimal256::from_str("0.998842965849973395").unwrap()
    );
    ...
    let last_price = helper
        ...
        .price_state
        .last_price;
    // 1.00118760696709103 (without tax)
    assert_eq!(
        last_price,
        Decimal256::from_str("1.001189240040121718").unwrap()
    );

    // Disable fee share
    let action = ConcentratedPoolUpdateParamsSaleTax::DisableFeeShare {};
    helper.update_config(&owner, &action).unwrap();
...
#[test]
fn check_sale_tax_change() {
    let owner = Addr::unchecked("owner");
    let tax_admin = Addr::unchecked("tax_admin");
    ...
    let random_user = Addr::unchecked("random");
    let action = ConcentratedPoolUpdateParamsSaleTax::UpdateSaleTax(SaleTaxConfigUpdates {
    ...
    helper.update_config(&owner, &action).unwrap();
    helper.update_config(&tax_admin, &action).unwrap();

    // updating sale tax admin
    helper
        .update_config(
            &tax_admin,
            &ConcentratedPoolUpdateParamsSaleTax::UpdateSaleTax(SaleTaxConfigUpdates {
                tax_config_admin: Some(random_user.to_string()),
                tax_configs: None,
            }),
        )
        .unwrap();
    helper.update_config(&random_user, &action).unwrap();
    let err = helper.update_config(&tax_admin, &action).unwrap_err();
    assert_eq!(ContractError::Unauthorized {}, err.downcast().unwrap());
    ...
    let err = helper
        .update_config(
            &random_user,
            &ConcentratedPoolUpdateParamsSaleTax::UpdateSaleTax(SaleTaxConfigUpdates {
                tax_config_admin: Some(tax_admin.to_string()),
                tax_configs: Some(TaxConfigs::from(vec![(
                    "unknown",
                    TaxConfig {
                        tax_rate: f64_to_dec(0.1),
                        tax_recipient: random_user.to_string(),
                    },
                )])),
            }),
        )
    ...
    let old_config = helper.query_config_smart().unwrap();
    let old_config: ConcentratedPoolConfigSaleTax =
        from_json(old_config.params.as_ref().unwrap()).unwrap();
    helper
        .update_config(
            &random_user,
            &ConcentratedPoolUpdateParamsSaleTax::UpdateSaleTax(SaleTaxConfigUpdates {
    ...
    let new_config = helper.query_config_smart().unwrap();
    let new_config: ConcentratedPoolConfigSaleTax =
        from_json(new_config.params.as_ref().unwrap()).unwrap();
    assert_eq!(
        new_config,
        ConcentratedPoolConfigSaleTax {
...
#[test]
fn check_swaps_simple_sale_tax() {
    let owner = Addr::unchecked("owner");
    let tax_recipient = Addr::unchecked("tax_recipient");
    ...
    assert_eq!(0, helper.coin_balance(&test_coins[1], &user));
    assert_eq!(0, helper.coin_balance(&test_coins[0], &tax_recipient));
    helper.swap(&user, &offer_asset, None).unwrap();
    // 99_737929 (without tax)
    assert_eq!(94_751169, helper.coin_balance(&test_coins[1], &user));
    assert_eq!(0, helper.coin_balance(&test_coins[0], &user));
    ...
    assert_eq!(
        5_000000,
        helper.coin_balance(&test_coins[0], &tax_recipient)
    );
