...
use astroport::asset::{user_input_to_addr, AssetInfo, PairInfo};
...
pub fn instantiate(
    ...
    let mut config = Config {
        ...
        fee_address: None,
        generator_address: user_input_to_addr(deps.api, msg.generator_address.as_str())?,
    };

    if let Some(fee_address) = msg.fee_address {
        config.fee_address = Some(user_input_to_addr(deps.api, fee_address.as_str())?);
    }
...
pub fn execute_update_config(
    ...
    if let Some(generator_address) = param.generator_address {
        // validate address format
        config.generator_address = user_input_to_addr(deps.api, generator_address.as_str())?;
    }
...
pub fn reply(deps: DepsMut, _env: Env, msg: Reply) -> Result<Response, ContractError> {
    ...
    let pair_contract = user_input_to_addr(deps.api, res.get_contract_address())?;
