...
async function generateTable(terra: LCDClient) {
    ...
    for (const key in network) {
        ...
        // each contract should saves with `address` substring name in .json config file
        if ( key.includes("address") ){
            await queryContractRaw(terra, end_point)
                .then(resp => {
                    let result = JSON.parse(toDecodedBinary(resp.data).toString());
                    console.table([{"locale name": `${key}`}, {"address": `${value}`},
                        {"runtime name": `${result.contract}`}, {"runtime version": `${result.version}`} ])
                })
